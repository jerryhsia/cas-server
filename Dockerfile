...
RUN cd / \
    && git clone -b latest --single-branch https://github.com/sergiofbsilva/cas-overlay-template.git cas-overlay \
    && mkdir /etc/cas \
    && mkdir /etc/cas/jetty \
    && mkdir -p cas-overlay/bin \
    && mkdir -p cas-overlay/src/main/webapp \
    && cp cas-overlay/etc/*.* /etc/cas \
    && cp cas-overlay/etc/jetty/thekeystore /etc/cas/jetty/;
